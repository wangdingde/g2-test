...
var Combo = function (input, util) {
	...
	return{
		...
		ctor: function (opts) {
			...
			this.bind({
				...
				onSetData: function (value) {
					this._setSelectedItem(this.getRecordByValue(value));
				}
			});
		...
		_timerHandler: function (value) {
			this.setValue(this.parse(value || this.getInputValue()));
		},
		_value2text: function (value) {
			var record = this.getRecordByValue(value),
				textField = this.textField;
			return record ? record[textField] : value;
		},
		...
		_initInput: function () {
			$$.InputBox.prototype._initInput.call(this);
			this.arrow = $("<span></span>").addClass("asc-combo-arrow").addClass(this._arrowicon).appendTo(this.dom)[0];
		...
		_initCombo: function () {
			...
			if (this.comboData) {
				this.loadComboData(this.comboData, true);
			}
		...
		loadComboData: function (data, must) {
			if (must || (data instanceof Array && data != this.comboData)) {
				this.comboData = data;
				...
				if (!this.combo) {
					...
					for (var i = 0, ii = data.length; i < ii; i++) {
						record = data[i];
						li = $("<li class=\"asc-combo-list-item " + (record[valueField] == this.value ? "asc-combo-list-selected" : "") + "\">" + record[textField] + "</li>").appendTo($ul)[0];
						li.record = record;
						record["-item"] = li;
					}
...
$$.define('form.Combo', ['form.InputBox', "core.Util"], Combo);

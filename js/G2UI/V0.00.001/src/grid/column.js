...
var column = function(uicontrol){
	return {
		...
		_initEditorProxy: function(rowIndex){
			...
			var el = this;
			$$.require([editor.xtype || "form.InputBox"], function(xtype){
				editor = el.editor = $$.create(xtype, editor);
				
				$(editor.dom).addClass("grid-column-editor");
		...
		_replaceEditor: function(rowIndex){
			...
			editor.setValue(grid.getCellData(rowIndex, this));
			editor.refreshShow();
		},
		initMenu: function(){
			if (this.groupCol) {

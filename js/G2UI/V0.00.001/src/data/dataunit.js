...
var du = function(ru, dr, dc){
	...
	return {
		...
		removeRow: function(row,sender) {
			...
			this.trigger("onRemoveRowed",sender);
		},
		...
		chkWidth: function(row,col,val) {
			col = this.getCol(col);
			if (col.auto != "" || $$.isEmptyString(val) || $$.isNull(val) || col.dtype == "Date" || col.totw == 0) {return;}
			if (val.toString().length > col.totw) {
		...
		newRow: function(vals,sender) {
			...
			//3 新增
			var row = this.addRow(); //里面會寫入tid和pid﹐并設置當前行

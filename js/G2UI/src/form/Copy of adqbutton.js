(function($$){
    var AdqButton = {
        ...
        initColumnData: function(){
        	...
        	if (columns.length === 0) {
        		for(var i = 0; i < cols.length; i++){
        			col = cols[i];
    				columnData.push({no: col.cname, name: this.getColCaption(col)});
        		}
        	} else {
        		for(var i = 0; i < columns.length; i++){
        			...
        			if(!col){continue;}
    				columnData.push({no: item.field, name: item.title || this.getColCaption(col)});
        		}
        	}
        ...
        initWin: function(){
        	...
        	$(queryTable).datagrid({
        		columns: [[
                   {field: 'column', title: '条件名', width: 50, editor: {
						type: "asc",
						options: {
							type: "Combo",
							editable: false,
                   ...
                   {field: 'opt', title: '比较符', width: 50, editor: {
						type: "asc",
						options: {
							type: "Combo",
							editable: false,
                   ...
                   {field: 'value', title: '条件值', width: 50, editor: 'asc'}
               ]]
            });
        	...
        	$$.create("Toolbar", {
				...
				userButtons: {
        			...
        			remove: {
        	            ...
        	            text: "删除条件",
        	            handler: function(){
            	        	if(queryUnit.isInputing){queryUnit.endInput(true);}
            		        ...
            		        if(queryUnit.isInputing){queryUnit.beginInput();}
        	            }
        			}
    			}
            });
        ...
        getQueryParmas: function(){
        	var unit = this.unit,
	        	...
	        	queryUnit = this.queryUnit,
	        	queryTable = this.table;
        	var whereStr = this.whereStr || "", params= this.params ? $.extend({},this.params) : {},
			...
			for(var i = 0; i < queryUnit.rows.length; i++){
				...
				if(isDate) {
					try{
						...
						$.messager.alert("警告", col.getCaption()+"日期格式不符合(年年年年月月日日)！", "warning");
						return;
					}
				}
				if(opt === "="){
				...
				whereStr += (whereStr ? " AND " : " ") + column + opt + ":" + colKey;
				params[colKey] = val;
			}
			return {
				whereStr: whereStr,
        ...
        _query: function(queryParams){
        	var unit = this.unit;
        	if(unit.mdl){
				$$.getRemoteData({accNo: "", clazz: "", name: unit.mdl.mname, opType: 1,whereStr:queryParams.whereStr,parmMap:queryParams.params },true);
			}
        },
        optData: [
        	{no: "=", name: "等于"},

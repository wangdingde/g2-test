...
var grid = function(UICONTROL, HEADCONTAINER, UTIL, KEYMANAGER, GRIDDATA, EDITTOOLBAR){
	return {
		...
		_init: function(){
			...
			if (!this.noHeader) {
				html += "<div class=\"panel-header\">" + (this.title || "New Grid") + "</div>";
			}
			if (this.ttbar) {
				html += "<div class=\"panel-toolbar grid-toolbar-top\"></div>";
			}
			html += "<div class=\"panel-body\"><div class=\"grid-editor\">"+
						"<table cellspacing=\"0\" cellpadding=\"0\"><tr></tr></table>"+
					"</div></div>";
			if (this.btbar) {
				html += "<div class=\"panel-toolbar grid-toolbar-bottom\"></div>";
			}
			if (!this.noFooter) {
				html += "<div class=\"panel-footer\">grid-footer</div>";
			}
			
			$dom.append(html);
			
			this.ttbar && (this.ttbar = this._initBar(this.ttbar, $(".grid-toolbar-top", $dom)));
			this.btbar && (this.btbar = this._initBar(this.btbar, $(".grid-toolbar-bottom", $dom)));
		...
		getView: function(pos){
			...
			var view = this.view,
				...
				dom = isMain ? $(".panel-body", this.dom)[0] : view["main"],
				css = "grid-view-" + pos,
				viewDom = view[pos];
				
			if (!dom) {
				dom = $(".panel-body", this.dom)[0];
				mth = "appendTo";
			}
		...
		_initTrEvents: function(index){
			...
			this.getTd(index).unbind(".g2ui.grid").bind({
				"click.g2ui.grid": function(){
					el.toFocused = el.getColumn(Number($(this).attr("col-index")));
				}
			});
		...
		endEdit: function(idx, sender){
			...
			for (i = 0; i < len; i++) {
				col = dataColumns[i];
				col.endEdit();
				if (col.editor && col.editor.getValue) {
					this.setData(idx, col, col.editor.getValue(), false);
					//col.editor.setValue(null, col);
					//col.editor.refreshShow();
				}
			}
			*/
		...
		setData: function(row, col, val, flag, sender){
			...
			if (flag && editor) {
				...
				this.getTd(this.getRowIndex(row), col).html(context.context)
					.removeClass("grid-td-error grid-td-change").addClass(context.css)
					.attr("title", context.errMsg);
			}
...
$$.define("grid.Grid2", ["core.UIControl", "grid.head.Container", "core.Util", "grid.KeyManager", "data.GridData", "tools.EditToolbar"], grid);
